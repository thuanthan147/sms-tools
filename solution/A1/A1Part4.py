...
def downsampleAudio(inputFile, M):
    ...
    """
    freq, audioMatrix = wavread(inputFile)
    print(len(audioMatrix))
    downMatrix = hopSamples(audioMatrix, M)
    newFreq = int(int(freq) / int(M))
    print(len(downMatrix)) 
    return newFreq, downMatrix 
    ## Your code here
freq, matrix = downsampleAudio("vibraphone-C6.wav",16)
wavwrite(matrix, freq, "vibraphone-C6_downsampled.wav")